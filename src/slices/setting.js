    ...
    reducers: {
        ...
        setOption: ( state, action ) => {
            return produce( state, draftState => {
                ...
                if ( keys[ keys.length - 1 ] === undefined && target[ keys[ keys.length - 1 ] ] === undefined ) {
                    target[ keys[ keys.length - 2 ] ] = {};
                    const add_key = keys[ keys.length - 1 ];
                    target[ keys[ keys.length - 2 ] ][ add_key ] = action.payload.value;
                } else {
